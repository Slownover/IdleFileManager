...
const ANSI_FG: Record<string, string> = {
    '30': '#4c4c4c', '31': '#cc0000', '32': '#4e9a06', '33': '#c4a000',
    '34': '#3465a4', '35': '#75507b', '36': '#06989a', '37': '#d3d7cf',
    '90': '#555753', '91': '#ef2929', '92': '#8ae234', '93': '#fce94f',
    '94': '#729fcf', '95': '#ad7fa8', '96': '#34e2e2', '97': '#eeeeec',
};

interface Span { text: string; style: React.CSSProperties; }
...
function parseAnsi(raw: string): Span[] {
    ...
    while ((m = re.exec(cleaned)) !== null) {
        if (m.index > last) out.push({ text: cleaned.slice(last, m.index), style: { ...cur } });
        ...
        if (!codes.length) { cur = {}; }
            ...
            codes.forEach(c => {
                ...
                else if (c === '22') { const { fontWeight: _fw, ...rest } = cur; cur = rest; }
                else if (c in ANSI_FG) cur = { ...cur, color: ANSI_FG[c] };
                else {
                    const n = parseInt(c, 10);
                    if ((n >= 40 && n <= 47) || (n >= 100 && n <= 107)) {
                        const fgKey = String(n - 10);
                        if (fgKey in ANSI_FG) cur = { ...cur, backgroundColor: ANSI_FG[fgKey] + '55' };
                    }
                }
            });
        }
    ...
    // eslint-disable-next-line no-control-regex
    const remainder = cleaned.slice(last).replace(/\x1b\[[0-9;]*[a-zA-Z]/g, '').replace(/\x1b/g, '');
...
function cleanLine(s: string): string {
    // eslint-disable-next-line no-control-regex
    return s.replace(/\x1b\[[^m]*m/g, '').replace(/\x1b\[[\d;]*[a-zA-Z]/g, '').replace(/\r/g, '');
}

interface LogLine { id: number; raw: string; }
let lid = 0;
...
const IdeConsole: React.FC = () => {
    ...
    const connect = useCallback(async () => {
        ...
        try {
            const { data } = await http.get(`/api/client/servers/${serverId}/websocket`);
                    ...
                    switch (msg.event) {
                        ...
                        case 'token expired':
                            http.get(`/api/client/servers/${serverId}/websocket`).then(r =>
                                ws.send(JSON.stringify({ event: 'auth', args: [r.data.data.token] }))
                            );
                            break;
                    }
                } catch { /* non-JSON */ }
            ...
            ws.onerror = () => { push('\x1b[91mWebSocket error.\x1b[0m'); setConnected(false); };
            ws.onclose = () => { if (wsRef.current === ws) { setConnected(false); push('\x1b[90mDisconnected.\x1b[0m'); } };
        } catch (e) {
    ...
    const onKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.key === 'ArrowUp')   { e.preventDefault(); const i = Math.min(histIdx + 1, history.length - 1); setHistIdx(i); setInput(history[i] ?? ''); return; }
    ...
    return (
        <div className="ide-console">
            <div ref={bodyRef} className="ide-console-output" onClick={() => inputRef.current?.focus()}>
                {lines.map(l => {
            ...
            <div className="ide-console-input-row">
                <span className="ide-console-prompt"><i className="bi bi-chevron-right"></i></span>
                <input
                    ref={inputRef}
                    ...
                    autoCapitalize="off"
                />
                <button className="ide-console-btn" onClick={copyLogs} title="Copy Logs">
                    <i className="bi bi-clipboard"></i>
                </button>
                <button className="ide-console-btn" onClick={() => setLines([])} title="Clear">
                    <i className="bi bi-trash3"></i>
                </button>
                <button className="ide-console-btn" onClick={connect} title="Reconnect">
                    <i className="bi bi-arrow-clockwise"></i>
                </button>
                <div className={`ide-console-dot ${connected ? 'ide-console-dot--ok' : 'ide-console-dot--err'}`} title={connected ? 'Connected' : 'Disconnected'} />
            </div>
        </div>
    );
};
