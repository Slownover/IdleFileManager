...
const IdeFileManager: React.FC<Props> = ({ children }) => {
    ...
    useEffect(() => { localStorage.setItem('ide-mode', isIdeMode ? 'true' : 'false'); }, [isIdeMode]);
    ...
    const [tabs, setTabs] = useState<IdeTab[]>(() => {
        try {
            ...
            if (cached) return JSON.parse(cached).map((t: any) => ({ ...t, content: null, isLoading: true, isDirty: false }));
        } catch {}
        return [];
    });
    const [activeTabId, setActiveTabId] = useState<string | null>(() => localStorage.getItem(`ide-active-tab-${serverId}`) || null);

    useEffect(() => {
        const mini = tabs.map(t => ({ id: t.id, name: t.name, path: t.path, pendingLine: t.pendingLine }));
    ...
    const onSidebarResize = (e: React.MouseEvent) => { e.preventDefault(); dragging.current = 'sidebar'; document.body.style.cursor = 'col-resize'; };
    const onConsoleResize = (e: React.MouseEvent) => { e.preventDefault(); dragging.current = 'console'; document.body.style.cursor = 'row-resize'; };
            ...
            {isIdeMode && (
                <div ref={rootRef} className="ide-root">
                    ...
                    <main ref={mainColRef} className="ide-main-col">
                        ...
                        {consoleOpen && (
                            <div className="ide-resizer ide-resizer--row" onMouseDown={onConsoleResize} />
                        )}
