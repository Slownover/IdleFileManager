...
const MediaViewer: React.FC<{ serverId: string; path: string; fileName: string }> = ({ serverId, path, fileName }) => {
    ...
    useEffect(() => {
        http.get(`/api/client/servers/${serverId}/files/download`, { params: { file: path } })
...
const LANG_MAP: Record<string, string> = {
    ...
    yaml: 'yaml', xml: 'xml', html: 'html', css: 'css', php: 'php',
    python: 'python', bash: 'shell', ini: 'ini', properties: 'ini',
    markdown: 'markdown', plaintext: 'plaintext', java: 'java',
    lua: 'lua', sql: 'sql', toml: 'ini', dockerfile: 'dockerfile',
...
const FileEditorTabs: React.FC<Props> = ({
    ...
    // ── 1. Load Monaco ──
    useEffect(() => {
        loadMonaco().then(() => setReady(true)).catch(console.error);
    }, []);
        ...
        <div className="ide-editor-panel">
            ...
            {tabs.length > 0 ? (
                <div className="ide-tabbar">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '0 12px', borderLeft: '1px solid var(--ide-border)' }}>
                        <button className="ide-statusbar__save-btn" onClick={onToggleMode} style={{ background: 'var(--ide-panel-bg)', border: '1px solid var(--ide-border)', marginRight: '8px' }} title="Switch to Classic Mode">
                            <i className="bi bi-box-arrow-right"></i>
                        </button>
                        <button className="ide-statusbar__save-btn" onClick={() => http.post(`/api/client/servers/${serverId}/power`, { signal: 'start' })} style={{ background: 'rgba(78,201,148,.2)', color: '#4ec994' }} title="Start Server"><i className="bi bi-play-fill"></i></button>
                        <button className="ide-statusbar__save-btn" onClick={() => http.post(`/api/client/servers/${serverId}/power`, { signal: 'restart' })} style={{ background: 'rgba(52,101,164,.2)', color: '#729fcf' }} title="Restart Server"><i className="bi bi-arrow-clockwise"></i></button>
                        <button className="ide-statusbar__save-btn" onClick={() => http.post(`/api/client/servers/${serverId}/power`, { signal: 'stop' })} style={{ background: 'rgba(244,135,113,.2)', color: '#f48771' }} title="Stop Server"><i className="bi bi-stop-fill"></i></button>
                    </div>
                </div>
            ) : (
                <div className="ide-tabbar">
                    ...
                    <div className="ide-statusbar__right">
                        ...
                        {activeTab.isDirty && !isBinary && (
                            <button className="ide-statusbar__save-btn" onClick={doSave} disabled={isSaving}>
                                <i className="bi bi-floppy"></i>{isSaving ? ' Saving…' : ' Save  Ctrl+S'}
                            </button>
                        )}
